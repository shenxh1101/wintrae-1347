...
def scan_cmd(
    ...
    _, records = _do_scan(directory, no_recursive, ignore, class_name, config)
...
def rename_cmd(
    ...
    renamed = sum(1 for _, p in results if p is not None)
    unchanged = sum(1 for _, p in results if p is None)

    if dry_run:
        console.print(f"\n[cyan]预览完成[/cyan]：将重命名 {renamed} 个文件"
                      + (f"，{unchanged} 个无需变更" if unchanged else ""))
    else:
        console.print(f"\n[green]✓[/green] 完成：已重命名 {renamed} 个文件"
                      + (f"，{unchanged} 个无需变更" if unchanged else ""))
...
def report_cmd(
    ...
    console.print("")
    console.print(f"  概览：共 {progress['total_records']} 个录音，"
    ...
    if comment:
        for r in records:
            if r.comment:
                ...
                r.comment = comment
        console.print(f"[cyan]ℹ[/cyan] 已追加评语标签：{comment}")

    splitter = Splitter(output_dir=output, copy=not move, date_range=date_range)
